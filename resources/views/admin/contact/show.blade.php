				...
				<div class="card" >
					...
					<div class="card shadow-lg p-4 bg-dark text-white text-center" style="width: 300px; margin: 0 auto; border-radius: 15px;">
						...
						<div class="card-footer mt-3">
							<a href="{{ route('contact.index') }}" class="btn btn-secondary btn-sm">Back</a>
													...
													@if ($contactus)
														@foreach ($contactus as $row)
															<tr>
																...
																<td>{{ $row->email }}</td>
																<td>
																	<img src="{{ asset($row->foto ?? '-') }}" alt="Foto" width="50">
																</td>
																<td>
																	<a href="{{ route('contact.edit', $row->id) }}" class="btn btn-success btn-sm me-2">
																		<i class="fas fa-edit"></i>
																	</a>
																	<form action="{{ route('contact.destroy', $row->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Yakin mau hapus?');">
																		@csrf
																		@method('DELETE')
